...
class TestLoadData:
    def test_load_data(self, tmp_path, sample_stock_data):
        ...
        with patch("os.path.basename", return_value="AAPL_1y_1d.csv"), patch(
            "os.path.splitext", return_value=("AAPL_1y_1d", ".csv")
        ):
...
class TestGeneratePredictionChart:
    def test_generate_prediction_chart(self, tmp_path, processed_stock_data):
        ...
        with patch("matplotlib.pyplot.savefig"), patch(
            ...
            return_value=str(
                pred_dir / f"{ticker}_{period}_{interval}_{model_name.lower()}.png"
            ),
        ):
    ...
    def test_date_calculation_for_different_intervals(self):
        ...
        data = pd.DataFrame({"Price": [pd.to_datetime("2023-01-15")], "Close": [150.0]})
        ...
        ), patch("matplotlib.pyplot.annotate"), patch("matplotlib.pyplot.title"), patch(
