...
class TestStockDataPipeline:
    ...
    def test_full_data_pipeline(
        ...
        os.makedirs(
            tmp_path / "data/predictions", exist_ok=True
        )  # Mock YFinance to return our sample test data
        mock_yf_download.return_value = sample_stock_data.rename(
            columns={"Price": "Date"}
        ).set_index("Date")
        ...
        with patch("os.makedirs"):
            with patch("builtins.open", create=True):
                ...
                processed_path = str(
                    tmp_path / f"data/processed/{ticker}_{period}_{interval}.csv"
                )
                ...
                with patch("pandas.read_csv", return_value=processed_stock_data):
                    with patch("pandas.DataFrame.to_csv"):
                        preprocessing.cleanData(
                            processed_stock_data, ticker, period, interval
                        )

                # Step 3: Add features
                with patch("pandas.read_csv", return_value=processed_stock_data):
                    with patch("pandas.DataFrame.to_csv"):
                        processed_data = processing.addFeatures(
                            processed_stock_data, ticker, period, interval
                        )

                # Step 4: Train a model and get prediction
                with patch(
                    "model.news.getSentimentData",
                    return_value=({"sentimentScore": 0.2, "enabled": True}, []),
                ):
                    with patch(
                        "model.news.adjustPredictionWithSentiment", return_value=165.0
                    ):
                        with patch(
                            "pandas.read_csv", return_value=processed_stock_data
                        ):
                            with patch(
                                ...
                                with patch("xgboost.XGBRegressor") as mock_xgb:
                                    ...
                                    x_train = processed_stock_data.drop(
                                        columns=["Price"]
                                    )
                                    y_train = pd.Series(
                                        [150.0] * len(processed_stock_data)
                                    )
                                    x_test = x_train.copy()
                                    y_test = pd.Series(
                                        [155.0] * len(processed_stock_data)
                                    )
                                    ...
                                    mock_model.predict.return_value = [160.0] * len(
                                        x_test
                                    )
    ...
    def test_prediction_to_chart_flow(
        self, mock_read_csv, mock_train_model, mock_chart, processed_stock_data
    ):
