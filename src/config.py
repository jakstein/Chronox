...
def loadConfig():
    ...
    return {
        ...
        "default_stock": {"ticker": "AAPL", "period": "1y", "interval": "1d"},
        "news_sentiment": {
            "enabled": True,
            "storiesCount": 20,
            "daysLookback": 7,
...
def validateArgs(period, interval, config=None):
    ...
    if period and period not in allowed_periods:
        return (
            False,
            f"Invalid period: '{period}'. Allowed periods are: {', '.join(allowed_periods)}",
        )

    if interval and interval not in allowed_intervals:
        return (
            False,
            f"Invalid interval: '{interval}'. Allowed intervals are: {', '.join(allowed_intervals)}",
        )
    ...
    if period in long_periods and interval in short_intervals:
        return (
            False,
            f"Short intervals ({', '.join(short_intervals)}) are not allowed with long periods ({', '.join(long_periods)}). Please select a longer interval (e.g., 1d, 5d, 1wk, 1mo).",
        )
