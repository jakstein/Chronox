...
def generatePredictionChart(
    data, predictionValue, error, days_ahead, ticker, period, interval, modelName
):
    # axos and figure
    ...
    plt.plot(
        [lastDate, futureDate],
        [close_prices.iloc[-1], predictionValue],
        color="red",
        linestyle="--",
        label=f"{modelName} Prediction",
    )
    ...
    plt.scatter(
        futureDate, predictionValue, color="red", s=50, zorder=5
    )  # ensure marker is on top

    # annotate predicted value
    plt.annotate(
        f"${predictionValue:.2f}",
    ...
    chartPath = (
        f"./data/predictions/{ticker}_{period}_{interval}_{modelName.lower()}.png"
    )
